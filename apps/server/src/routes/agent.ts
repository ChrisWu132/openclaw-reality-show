...
/**
 * POST /api/agent/from-memory
 * Asks the OpenClaw service to synthesize a new agent personality from the
 * local user's Claude memory files, then returns the agentId.
 *
 * The frontend calls this before creating a session so the session can be
 * linked to a memory-derived agent. Proxied through the game server so the
 * frontend never needs to know about the OpenClaw service directly.
 */
